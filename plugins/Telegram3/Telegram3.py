...
"""
Plugin to send FMS-, ZVEI- and POCSAG-messages via Telegram3
@author: Peter Laemmle
@requires: Telegram3 BOT token, Telegram3 chat ID, library python-telegram-bot and optional requests and json
"""
...
if globalVars.config.get("Telegram3","RICforLocationAPIKey3"):
...
def onLoad():
	...
	configHandler.checkConfig("Telegram3")
	BOTTokenAPIKey = globalVars.config.get("Telegram3","BOTTokenAPIKey3")
	BOTChatIDAPIKey = globalVars.config.get("Telegram3","BOTChatIDAPIKey3")
	RICforLocationAPIKey = globalVars.config.get("Telegram3","RICforLocationAPIKey3")
	GoogleAPIKey = globalVars.config.get("Telegram3","GoogleAPIKey3")
	RoutingOrigin = globalVars.config.get("Telegram3","RoutingOrigin3")
	...
	try:
		...
		try:
			if typ in ("POC", "FMS", "ZVEI"):
				...
				text = globalVars.config.get("Telegram3", "%s_message" % typ)
				text = wildcardHandler.replaceWildcards(text, data)

				# Initiate Telegram Bot
				logging.debug("Initiate Telegram3 BOT")
				bot = telegram.Bot(token='%s' % BOTTokenAPIKey)
				# Send message to chat via Telegram BOT API
				logging.debug("Send message to chat via Telegram3 BOT API")
				...
				if typ == "POC" and data["ric"] == RICforLocationAPIKey:
					...
					logging.debug("Send message and maps via Telegram3 BOT")
					...
					logging.debug("Send location via Telegram3 BOT API")
			...
			logging.error("Telegram3 Error: Unauthorized")
			logging.debug("Telegram3 Error: Unauthorized", exc_info=True)
		except BadRequest:
			logging.error("Telegram3 Error: BadRequest")
			logging.debug("Telegram3 Error: BadRequest", exc_info=True)
		except NetworkError:
			logging.error("Telegram3 Error: NetworkError")
			logging.debug("Telegram3 Error: NetworkError", exc_info=True)
		except TelegramError:
			logging.error("Telegram3 Error: TelegramError")
			logging.debug("Telegram3 Error: TelegramError", exc_info=True)
		########## User Plugin CODE ##########
