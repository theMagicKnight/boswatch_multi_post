...
"""
Plugin to send FMS-, ZVEI- and POCSAG-messages via Telegram2
@author: Peter Laemmle
@requires: Telegram2 BOT token, Telegram2 chat ID, library python-telegram-bot and optional requests and json
"""
...
if globalVars.config.get("Telegram2","RICforLocationAPIKey2"):
...
def onLoad():
	...
	configHandler.checkConfig("Telegram2")
	BOTTokenAPIKey = globalVars.config.get("Telegram2","BOTTokenAPIKey2")
	BOTChatIDAPIKey = globalVars.config.get("Telegram2","BOTChatIDAPIKey2")
	RICforLocationAPIKey = globalVars.config.get("Telegram2","RICforLocationAPIKey2")
	GoogleAPIKey = globalVars.config.get("Telegram2","GoogleAPIKey2")
	RoutingOrigin = globalVars.config.get("Telegram2","RoutingOrigin2")
	...
	try:
		...
		try:
			if typ in ("POC", "FMS", "ZVEI"):
				...
				text = globalVars.config.get("Telegram2", "%s_message" % typ)
				text = wildcardHandler.replaceWildcards(text, data)

				# Initiate Telegram Bot
				logging.debug("Initiate Telegram2 BOT")
				bot = telegram.Bot(token='%s' % BOTTokenAPIKey)
				# Send message to chat via Telegram BOT API
				logging.debug("Send message to chat via Telegram2 BOT API")
				...
				if typ == "POC" and data["ric"] == RICforLocationAPIKey:
					...
					logging.debug("Send message and maps via Telegram2 BOT")
					...
					logging.debug("Send location via Telegram2 BOT API")
			...
			logging.error("Telegram2 Error: Unauthorized")
			logging.debug("Telegram2 Error: Unauthorized", exc_info=True)
		except BadRequest:
			logging.error("Telegram2 Error: BadRequest")
			logging.debug("Telegram2 Error: BadRequest", exc_info=True)
		except NetworkError:
			logging.error("Telegram2 Error: NetworkError")
			logging.debug("Telegram2 Error: NetworkError", exc_info=True)
		except TelegramError:
			logging.error("Telegram2 Error: TelegramError")
			logging.debug("Telegram2 Error: TelegramError", exc_info=True)
		########## User Plugin CODE ##########
